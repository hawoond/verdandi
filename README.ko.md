...
Verdandi는 자연어 요청을 외부 LLM 코딩 에이전트가 사용할 workflow package로
바꾸는 순수 Go 기반 MCP 런타임입니다. CLI와 MCP stdio 서버를 함께 제공하므로
터미널과 MCP 지원 LLM 클라이언트에서 같은 agent/skill asset registry를 사용할
수 있습니다.

## 무엇을 하나요?

- 자연어 요청을 분석해 LLM 코딩 에이전트가 사용할 workflow package를 생성합니다.
- 실제 애플리케이션 코드는 Verdandi가 직접 생성하지 않습니다. 코드는 외부 LLM 코딩 에이전트가 작성합니다.
- 요청에 맞는 agent asset과 skill asset을 동적으로 생성하거나 기존 asset을 재사용합니다.
- 생성된 agent와 skill은 삭제되지 않고 `.verdandi/registry/assets.json`에 저장됩니다.
- 이후 작업에서 기존 asset을 검색하고, 성공률과 실패 기록을 바탕으로 재사용, 개선, 분리, 비추천을 판단합니다.
- `prepare_workflow`, `recommend_assets`, `record_outcome`, `list_agents`, `list_skills` MCP 도구를 제공합니다.
- workflow handoff, selected assets, task graph를 `.verdandi/workflows/{runId}/` 아래에 저장합니다.
...
Verdandi는 workflow package, 실행 기록, agent/skill metric, lifecycle outcome,
visualization event를 `.verdandi/` 아래에 기록합니다. MCP 리소스는 optional
Spinning Wheel 시각화 도구가 스트리밍하는 동일한 상태를 노출합니다. 설정과 실행
방법은 [docs/spinning-wheel.md](docs/spinning-wheel.md)를 참고하세요.

## 현재 범위

Verdandi는 현재 로컬 MCP 기반 asset orchestrator입니다. 애플리케이션 소스 코드를
직접 작성하지 않고, 외부 LLM 코딩 에이전트가 사용할 handoff, selected assets,
task graph, outcome 기록 표면에 집중합니다.
