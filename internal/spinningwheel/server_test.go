...
func TestServerServesSpinningWheelUI(t *testing.T) {
	...
	for _, expected := range []string{"playPauseButton", "stepButton", "speedRange", "eventCounter", "agentRoster", "conversationLog"} {
	...
	for _, expected := range []string{"requestAnimationFrame", "moveAgentTo", "spawnedAt", "agent-spawned", "speechForEvent", "drawDecisionLinks", "activeStage", "stepForward", "playbackDelay", "renderAgentRoster", "appendConversation"} {
